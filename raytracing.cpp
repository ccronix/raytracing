...
vec3d trace(const group& grp, const ray& r, object* lights,  int depth)
{
    if (depth <= 0) {
        return vec3d(0, 0, 0);
    }
    ...
    if (scattering.is_spec) {
        return scattering.attenuation * trace(grp, scattering.specular, lights, depth - 1);
    }
    ...
    return emission + scattering.attenuation * crossover.mat->shading_pdf(r, crossover, next_r) * trace(grp, next_r, lights, depth - 1) / pdf_value;
}
    ...
    for (int i = 0; i < height; i++) {
        ...
        for (int j = 0; j < width; j++) {
            ...
            for (int k = 0; k < spp; k++) {
                ...
                ray r = cam.emit(x, y);
                vec3d sample = trace(grp, r, lights, max_depth);
