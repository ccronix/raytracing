...
__device__ group random_scene(curandState* state)
{
    ...
    for(int i = -11; i < 11; i++) {
        for (int j = -11; j < 11; j++) {
            double choose = random_double(state);
            vec3d center = vec3d(i + 0.9 * random_double(state), 0.2, j + 0.9 * random_double(state));
...
__device__ group all_feature_test(curandState* state)
{
    group grp;
    group ground_boxes;
    ...
    for (int i = 0; i < 20; i++) {
        for (int j = 0; j < 20; j++) {
            ...
            double x1 = x0 + w;
            double z1 = z0 + w;
            double y1 = random_double(1, 101, state);
    ...
    for (int i = 0; i < 1000; i++) {
        vol_boxes.add(new sphere(random_vector(0, 165, state), 10, white_mat));
    }
