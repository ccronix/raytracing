...
public:
    __device__ triangle(vertex* vertices, material* mat) 
    {
    ...
    __device__ triangle(vec3d p0, vec3d p1, vec3d p2, material* mat)
    {
        this->vertices = (vertex*) malloc(sizeof(vertex) * 3);
    ...
    __device__ virtual ~triangle() override
    {
        free(vertices);
        free(mat);
        vertices = nullptr;
    ...
    {
        vec3d point;
        double u = random_double(state);
        double v = random_double(state);
...
private:
    ...
    __device__ vec2d interpret_uv(double u, double v) const
    {
        return u * vertices[1].uv_coord + v * vertices[2].uv_coord + (1 - u - v) * vertices[0].uv_coord;
    }

    __device__ vec3d interpret_normal(double u, double v) const
    {
