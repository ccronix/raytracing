...
public:
    ...
    __device__ sphere(vec3d center, double radius, material* mat) 
    {
        this->center = center;
        this->radius = radius;
        this->mat = mat;
...
private:
    __device__ static vec2d uv(const vec3d& position)
    {
        ...
        double phi = atan2(-position.z(), position.x()) + 3.141592653589;
        double u = phi / (2 * 3.141592653589);
        double v = theta / 3.141592653589;
        return vec2d(u, v);
    }
};
...
public:
    ...
    __device__ msphere(vec3d center0, vec3d center1, double start, double end, double radius, material* mat) 
    {
    ...
    __device__ vec3d center(double time) const
    {
...
public:
    ...
    __device__ planexy(double x0, double x1, double y0, double y1, double k, material* mat)
    {
        this->x0 = x0;
        this->x1 = x1;
        this->y0 = y0;
...
public:
    ...
    __device__ planexz(double x0, double x1, double z0, double z1, double k, material* mat)
    {
        this->x0 = x0;
        this->x1 = x1;
        this->z0 = z0;
    ...
    {
        vec3d point = vec3d(random_double(x0, x1, state), k, random_double(z0, z1, state));
        return point - origin;
    }
};
...
public:
    ...
    __device__ planeyz(double y0, double y1, double z0, double z1, double k, material* mat)
    {
        this->y0 = y0;
        this->y1 = y1;
        this->z0 = z0;
...
public:
    __device__ volume(object* surface, double density, texture* tex)
    {
    ...
    __device__ volume(object* surface, double density, const vec3d& color)
    {
        this->surface = surface;
        mat = new fog(color);
        neg_inv_d = -1.0 / density;
...
public:
    ...
    __device__ box(const vec3d& minimum, const vec3d& maximum, material* mat)
    {
    ...
    __device__ virtual ~box() override
    {
        delete xy0;
        delete xy1;
        delete xz0;
